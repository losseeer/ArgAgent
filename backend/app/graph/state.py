...
class DebateState(TypedDict):
    # —— 身份与追踪 ——
    session_id: str
    user_id: str                        # 服务端生成的 device-local id；任何端点不接受客户端传来的同名字段（Q7）
    trace_id: str
    turn_index: int
    topic: str
    ...
    agent_self_contradiction: Optional[Dict]  # {turn_i, turn_j}

    # —— 出口校验与安全（§6.4#9 / §6.2 OUTPUT 行）——
    validation: Dict                    # {passed, missing[], attempt}；missing 枚举见 §6.11
    safety: Optional[Dict]              # {mode, input_hit, output_hit}——SSE safety.* 与 N8 的唯一数据源
